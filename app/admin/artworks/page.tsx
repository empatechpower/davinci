...
function LabeledInput({
  ...
  return (
    <div
      className={`border border-ad-border rounded-[8px] px-3 pt-2 pb-2 bg-white ${className}`}
    >
...
function ArtworkForm({
  ...
  artwork?: Artwork;
  initState?: Partial<ArtworkFormState>;
  artists: Artist[];
  onBack: () => void;
  onSaved: () => void;
}) {
  const [form, setForm] = useState<ArtworkFormState>({
    ...BLANK,
    ...initState,
  });
  ...
  const set = (key: keyof ArtworkFormState) => (v: string) =>
    setForm((p) => ({ ...p, [key]: v }));
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-w-[780px]">
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onBack}
          className="p-1 text-ad-dark hover:text-ad-purple transition-colors"
        >
      ...
      <div className="bg-white border border-ad-border rounded-[16px] p-6 flex flex-col gap-5">
        ...
        <section className="flex flex-col gap-3">
          <h3 className="text-[14px] font-semibold text-ad-dark">
            Product Details
          </h3>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <LabeledInput
              label="Product Type"
              placeholder="e.g., A3 Metal Frame"
          ...
          <div className="border border-ad-border rounded-[8px] px-3 pt-2 pb-2 bg-white">
            <p className="text-[12px] font-semibold text-ad-dark mb-0.5">
              Artist
            </p>
            <select
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="border border-ad-border rounded-[8px] px-3 pt-2 pb-2 bg-white">
            ...
            <textarea
              ...
              rows={4}
              className="w-full text-[13px] text-ad-dark placeholder:text-gray-400 outline-none bg-transparent resize-none"
            />
          </div>
          <div className="border border-ad-border rounded-[8px] px-3 pt-2 pb-2 bg-white">
            <p className="text-[12px] font-semibold text-ad-dark mb-0.5">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex flex-wrap gap-2">
                {artwork.photos.map((url, i) => (
                  <div
                    key={i}
                    className="w-16 h-16 rounded-[6px] overflow-hidden border border-ad-border"
                  >
                    <Image
                      src={url}
                      alt={`photo ${i + 1}`}
          ...
          {form.photoPreviews.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {form.photoPreviews.map((src, i) => (
                <div
                  key={i}
                  className="relative w-20 h-20 rounded-[6px] overflow-hidden border border-ad-border"
                >
                  <Image
                    src={src}
                    alt={`preview ${i + 1}`}
        ...
        <section className="flex flex-col gap-3">
          ...
          {isEdit && artwork?.videos && artwork.videos.length > 0 && (
            <div className="flex flex-col gap-1">
              <p className="text-[11px] text-ad-gray">
                {artwork.videos.length} video(s) already uploaded
              </p>
            </div>
          )}
          ...
          {form.videoNames.length > 0 && (
            <div className="flex flex-col gap-1.5">
              {form.videoNames.map((name, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 bg-gray-50 border border-ad-border rounded-[8px] px-3 py-2"
                >
                  <span className="text-[13px] text-ad-dark flex-1 truncate">
                    {name}
                  </span>
                  <button
                    type="button"
  ...
  return (
    <>
      <div className="flex flex-col gap-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[12px] text-ad-gray">
              Browse and manage all artworks in the collection
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-3 gap-4">
          {stats.map(({ label, value }) => (
            <div
              key={label}
              className="bg-white border border-ad-border rounded-[16px] px-6 py-5"
            >
              <p className="text-[12px] text-ad-gray font-medium">{label}</p>
              <p className="text-[32px] font-bold text-ad-purple leading-none mt-1">
                {value}
              </p>
            </div>
          ))}
                ...
                >
                  <div className="aspect-square relative overflow-hidden bg-as-card">
                    {thumbUrl ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-4xl text-ad-purple/30 font-light">
                        {artwork.title[0]}
                      </div>
                    )}
                    {deleting === artwork.id && (
                  ...
                  <div className="p-3">
                    <p className="text-[13px] font-medium text-ad-dark truncate">
                      {artwork.title}
                    </p>
                    <p className="text-[11px] text-ad-gray truncate">
                      {artist?.name ?? "Unknown artist"}
                    </p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-[12px] bg-ad-purple/10 text-ad-purple px-2 py-0.5 rounded-full truncate max-w-[100px]">
                        {artwork.dimensions || artwork.category || "—"}
                      </span>
                      <span className="text-[13px] font-semibold text-ad-dark">
                        ${artwork.price ?? "—"}
                      </span>
                    </div>
                  </div>
      ...
      {selected && (
        ...
        >
          <div
            className="bg-white rounded-[16px] w-full max-w-[580px] max-h-[90vh] overflow-y-auto shadow-2xl"
          ...
          >
            ...
            <div className="relative aspect-video bg-as-card">
              {(selected.photos?.[0] ?? selected.image_url) ? (
                <Image
                  src={selected.photos?.[0] ?? selected.image_url!}
                  alt={selected.title}
                  fill
                  className="object-cover"
                />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-6xl text-ad-purple/20">
                  {selected.title[0]}
                </div>
              )}
              <button
                onClick={() => setSelected(null)}
                className="absolute top-3 right-3 w-8 h-8 bg-white/80 rounded-full flex items-center justify-center text-ad-dark hover:bg-white"
              >
                ✕
              </button>
            </div>
            ...
            {selected.photos && selected.photos.length > 1 && (
              <div className="flex gap-2 px-4 py-2 overflow-x-auto">
                {selected.photos.slice(1).map((url, i) => (
                  <div
                    key={i}
                    className="w-14 h-14 shrink-0 rounded-[6px] overflow-hidden border border-ad-border"
                  >
                    <Image
                      src={url}
                      alt={`photo ${i + 2}`}
            ...
            <div className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-[18px] font-bold text-ad-dark">
                    {selected.title}
                  </h3>
                  <p className="text-[13px] text-ad-gray mt-0.5">
                    by{" "}
                    {artists.find((a) => a.id === selected.artist_id)?.name ??
                      "Unknown"}
                  </p>
                  {selected.sku && (
                    <p className="text-[11px] text-ad-gray mt-0.5">
                      SKU: {selected.sku}
                    </p>
                  )}
                </div>
                <p className="text-[20px] font-bold text-ad-purple shrink-0">
                  ${selected.price ?? "—"}
                </p>
              </div>
              <div className="flex flex-wrap gap-2 mt-3">
                {selected.category && (
                  <span className="text-[12px] bg-ad-purple/10 text-ad-purple px-2 py-0.5 rounded-full">
                    {selected.category}
                  </span>
                )}
                ...
                {selected.dimensions && (
                  <span className="text-[12px] bg-gray-100 text-ad-gray px-2 py-0.5 rounded-full">
                    {selected.dimensions}
                  </span>
                )}
              </div>
              {selected.description && (
                <p className="mt-4 text-[13px] text-ad-gray leading-relaxed">
                  {selected.description}
                </p>
              )}
              {selected.remarks && (
                <p className="mt-2 text-[13px] text-ad-gray leading-relaxed italic">
                  {selected.remarks}
                </p>
              )}
              {selected.videos && selected.videos.length > 0 && (
                <div className="mt-4 flex flex-col gap-3">
                  <p className="text-[12px] font-semibold text-ad-dark">
                    Videos
                  </p>
